    ...
    float4* kernel_ptr, cudaStream_t* S_ker, const size_t dst_dex)
{
    ...
    if (total_len > REDUCTION_VEC4_BLOCK)
    {
        grid = decx::utils::ceil<size_t>(thr_num, REDUCTION_VEC4_BLOCK);
        
        cu_sConv2_LK_start << <grid, REDUCTION_VEC4_BLOCK, 0, *S_ker >> > (
        ...
        while (1) {
            if (decx::utils::ceil<size_t>(thr_num, REDUCTION_VEC4_BLOCK) == 1) { break; }
            grid = decx::utils::ceil<size_t>(thr_num, REDUCTION_VEC4_BLOCK);

            if (dev_tmp1->leading) {
                cu_sConv2_LK << <grid, REDUCTION_VEC4_BLOCK, 0, *S_ker >> > (dev_tmp1->mem, dev_tmp2->mem, thr_num);
                decx::utils::set_mutex_memory_state<float4, float4>(dev_tmp2, dev_tmp1);
            }
            else {
                cu_sConv2_LK << <grid, REDUCTION_VEC4_BLOCK, 0, *S_ker >> > (dev_tmp2->mem, dev_tmp1->mem, thr_num);
        ...
        thr_num = grid / 2;
        if (dev_tmp1->leading) {
            cu_sConv2_LK_end << <1, REDUCTION_VEC4_BLOCK, 0, *S_ker >> > (dev_tmp1->mem, dst_ptr, thr_num, dst_dex);
            decx::utils::set_mutex_memory_state<float4, float4>(dev_tmp2, dev_tmp1);
        }
        else {
            cu_sConv2_LK_end << <1, REDUCTION_VEC4_BLOCK, 0, *S_ker >> > (dev_tmp2->mem, dst_ptr, thr_num, dst_dex);
            decx::utils::set_mutex_memory_state<float4, float4>(dev_tmp1, dev_tmp2);
        }
    }
}
