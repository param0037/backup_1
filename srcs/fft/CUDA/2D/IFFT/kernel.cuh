...
#ifndef _IFFT2D_KERNEL_CUH_
#define _IFFT2D_KERNEL_CUH_
...
__global__
void cu_IFFT2D_b2_CC_halved(float4*         src,
                            float4*         dst,
                            const int       thr_num,
...
*/
void cu_IFFT2D_b2_single_clamp_last(float2*         src,
                                    float2*         dst,
                                    const int       warp_proc_len,
                                    const int       warp_len,
                                    const int       thr_num,
...
//#ifdef __CUDACC__
    ...
    if (tidy < thr_num && tidx < height_limit) {
        ...
        //dst[map_dex + dex_base] = tmp;
        
        tmp.x = __fdiv_rn(tmp.x, __int2float_rn(Wsrc));
        tmp.y = __fdiv_rn(tmp.y, __int2float_rn(Wsrc));

        dst[map_dex * height_limit + tidx] = tmp;

        C_SUB_C(mid_term[0], mid_term[1], tmp);
        //dst[map_dex + warp_len + dex_base] = tmp;
...
* @brief : A transpose operation is included
*/
void cu_IFFT2D_b2_single_clamp_last_R(float2*         src,
                                      float*          dst,
                                      const int       warp_proc_len,
                                      const int       warp_len,
                                      const int       thr_num,
...
//#ifdef __CUDACC__
    ...
    if (tidy < thr_num && tidx < height_limit) {
        ...
        mid_term[0] = src[tidy * 2 + dex_base];
        tmp.x = __fadd_rn(mid_term[0].x, mid_term[1].x);
        
        tmp.x = __fdiv_rn(tmp.x, __int2float_rn(Wsrc));
        dst[map_dex * height_limit + tidx] = tmp.x;

        tmp.x = __fsub_rn(mid_term[0].x, mid_term[1].x);
        tmp.x = __fdiv_rn(tmp.x, __int2float_rn(Wsrc));
        dst[(map_dex + warp_len) * height_limit + tidx] = tmp.x;
    }
//#endif
}
...
*/
void cu_IFFT2D_b3_CC_halved(float4*         src,
                           float4*         dst,
                           const int       thr_num,
...
* real part into the right place, because the rotation factor is W(k, 0) = 1
*/
void cu_IFFT2D_b3_CC(float2*         src,
                     float2*         dst,
                     const int       thr_num,
                     const int       warp_proc_len,            // 一个warp所处理的数组长度 = padded
                     const int       warp_len,
...
* real part into the right place, because the rotation factor is W(k, 0) = 1
*/
void cu_IFFT2D_b3_CC_trans(float2*         src,
                           float2*         dst,
                           const int       thr_num,
                           const int       warp_proc_len,            // 一个warp所处理的数组长度 = padded
                           const int       warp_len,
...
//#ifdef __CUDACC__
    ...
    if (is_in)
    {
        ...
        tmp.x = __fdiv_rn(tmp.x, Wsrc);
        tmp.y = __fdiv_rn(tmp.y, Wsrc);
...
* real part into the right place, because the rotation factor is W(k, 0) = 1
*/
void cu_IFFT2D_b3_CC_trans_R(float2*         src,
                             float*          dst,
                             const int       thr_num,
                             const int       warp_proc_len,             // 一个warp所处理的数组长度 = padded
                             const int       warp_len,
...
//#ifdef __CUDACC__
    ...
    if (is_in)
    {
        ...
        tmp.x = __fdiv_rn(tmp.x, Wsrc);
        dst[init_dex * height_limit + tidx] = tmp.x;
        ...
        tmp.x = __fdiv_rn(tmp.x, Wsrc);
        dst[(init_dex + warp_len) * height_limit + tidx] = tmp.x;
        ...
        tmp.x = __fdiv_rn(tmp.x, Wsrc);
        dst[(init_dex + (warp_len << 1)) * height_limit + tidx] = tmp.x;
    }
//#endif
}
...
*/
void cu_IFFT2D_b4_CC_halved(float4*          src,
                           float4*          dst,
                           const int        thr_num,
                           const int        warp_proc_len,
                           const int        warp_len,
...
*/
void cu_IFFT2D_b4_CC_consec128_halved(float4*         src,
                                      float4*         dst,
...
* spread some threads on row dimension to process some datas
*/
void cu_IFFT2D_b5_CC_halved(float4*         src,
                            float4*         dst,
                            const int       thr_num,
                            const int       warp_proc_len,
                            const int       warp_len,
...
* spread some threads on row dimension to process some datas
*/
void cu_IFFT2D_b5_CC(float2*         src,
                     float2*         dst,
                     const int       thr_num,
                     const int       warp_proc_len,
                     const int       warp_len,
...
* spread some threads on row dimension to process some datas
*/
void cu_IFFT2D_b5_CC_trans(float2*         src,
                           float2*         dst,
                           const int       thr_num,
                           const int       warp_proc_len,
                           const int       warp_len,
...
* spread some threads on row dimension to process some datas
*/
void cu_IFFT2D_b5_CC_trans_R(float2*         src,
                             float*          dst,
                             const int       thr_num,
                             const int       warp_proc_len,
                             const int       warp_len,
    ...
    if (is_in)
    {
        ...
        */
        ...
        // the fourth one
        {
            ...
            tmp.x = __fdiv_rn(tmp.x, Wsrc);
            dst[(init_dex + (warp_len * 3)) * height_limit + tidx] = tmp.x;
        }
        // the fifth one
        {
            tmp = CmulC_buffer[0];        // reset
            ...
            tmp.x = __fdiv_rn(tmp.x, Wsrc);
            dst[(init_dex + (warp_len << 2)) * height_limit + tidx] = tmp.x;
        }
    }
//#endif
}
...
#endif      // #ifndef _IFFT2D_KERNEL_CUH_