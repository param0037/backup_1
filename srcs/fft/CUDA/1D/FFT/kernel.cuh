...
#ifndef _FFT1D_KERNEL_CUH_
#define _FFT1D_KERNEL_CUH_
...
__global__
void cu_FFT1D_b2_single_clamp_last(float2*         src,
                                   float2*         dst,
...
__global__
void cu_FFT1D_b2_CC_halved(float4*         src,
                           float4*         dst,
                           const int       thr_num,
                           const int       warp_proc_len,
                           const int       warp_len)
...
// because it is in the middle, do not need to worry about CC or RC, it is always CC mode
__global__
void cu_FFT1D_b2_CC(float2*         src,
                    float2*         dst,
                    const int       thr_num,
                    const int       warp_proc_len,
                    const int       warp_len)
...
*/
void cu_FFT1D_b3_CC_halved(float4*         src,
                           float4*         dst,
                           const int       thr_num,
...
* real part into the right place, because the rotation factor is W(k, 0) = 1
*/
void cu_FFT1D_b3_CC(float2*         src,
                    float2*         dst,
                    const int       thr_num,
                    const int       warp_proc_len,            // 一个warp所处理的数组长度 = padded
                    const int       warp_len)                 // 线程蔟长度
...
*/
void cu_FFT1D_b4_CC_halved(float4*     src,
                           float4*     dst,
                           const int   thr_num,
                           const int   warp_proc_len,
                           const int   warp_len)
...
* spread some threads on row dimension to process some datas
*/
void cu_FFT1D_b4_CC(float2*     src,
                    float2*     dst,
                    const int   thr_num,
                    const int   warp_proc_len,
                    const int   warp_len)
...
*/
void cu_FFT1D_b4_CC_consec128_halved(float4*         src,
                                     float4*         dst,
                                     ...
                                     const int       __iter)
{
    ...
    if (threadIdx.x < thread_num_limit) 
    {
        ...
        /*local_fragment[threadIdx.x * 4] = make_float4(1, 1, 1, 1);
        local_fragment[threadIdx.x * 4 + 1] = make_float4(1, 1, 1, 1);
        local_fragment[threadIdx.x * 4 + 2] = make_float4(1, 1, 1, 1);
        local_fragment[threadIdx.x * 4 + 3] = make_float4(1, 1, 1, 1);*/
...
* spread some threads on row dimension to process some datas
*/
void cu_FFT1D_b5_CC_halved(float4*     src,
                           float4*     dst,
                           const int   thr_num,
                           const int   warp_proc_len,
                           const int   warp_len)
...
* spread some threads on row dimension to process some datas
*/
void cu_FFT1D_b5_CC(float2*     src,
                    float2*     dst,
                    const int   thr_num,
                    const int   warp_proc_len,
                    const int   warp_len)
...
#endif          // #ifndef _FFT1D_KERNEL_CUH_