...
namespace decx
{
    void Sum_fp32_vec(decx::_Vector<float>* src, float* res, de::DH* handle);

    void Sum_fp32_mat(_Matrix<float>* src, float* res, de::DH* handle);
}


namespace de
{
    _DECX_API_ de::DH Global_Sum(de::Vector<float>& src, float* res);

    _DECX_API_ de::DH Global_Sum(de::Matrix<float>& src, float* res);
}




void decx::Sum_fp32_vec(decx::_Vector<float>* src, float* res, de::DH* handle)
{
    ...
    while (1) {
        ...
        if (dev_A.leading) {
            cu_sum_vec4f << <grid, REDUCTION_VEC4_BLOCK, 0, S >> > (dev_A.mem, dev_B.mem, thr_num);
            decx::utils::set_mutex_memory_state<float4, float4>(&dev_B, &dev_A);
        }
        else {
            cu_sum_vec4f << <grid, REDUCTION_VEC4_BLOCK, 0, S >> > (dev_B.mem, dev_A.mem, thr_num);
            decx::utils::set_mutex_memory_state<float4, float4>(&dev_A, &dev_B);
        }
        thr_num = grid / 2;
...
void decx::Sum_fp32_mat(_Matrix<float>* src, float* res, de::DH* handle)
{
    // 2x float4 as a pair of adding operation
...
de::DH de::Global_Sum(de::Vector<float>& src, float* res)
{
    ...
    decx::Sum_fp32_vec(_src, res, &handle);
    return handle;
}


de::DH de::Global_Sum(de::Matrix<float>& src, float* res)
{
    ...
    decx::Sum_fp32_mat(_src, res, &handle);
    return handle;
}