...
namespace decx
{
    void Max_fp32_vec(decx::_Vector<float>* src, float* res, de::DH* handle);

    void Max_fp32_mat(_Matrix<float>* src, float* res, de::DH* handle);
}


namespace de
{
    _DECX_API_ de::DH Global_Max(de::Vector<float>& src, float* res);

    _DECX_API_ de::DH Global_Max(de::Matrix<float>& src, float* res);
}




void decx::Max_fp32_vec(decx::_Vector<float>* src, float* res, de::DH* handle)
{
    ...
    while (1) {
        ...
        if (dev_A.leading) {
            cu_max_vec4f << <grid, REDUCTION_VEC4_BLOCK, 0, S >> > (dev_A.mem, dev_B.mem, thr_num, src->Vec.ptr[0]);
            decx::utils::set_mutex_memory_state<float4, float4>(&dev_B, &dev_A);
        }
        else {
            cu_max_vec4f << <grid, REDUCTION_VEC4_BLOCK, 0, S >> > (dev_B.mem, dev_A.mem, thr_num, src->Vec.ptr[0]);
            decx::utils::set_mutex_memory_state<float4, float4>(&dev_A, &dev_B);
        }
        thr_num = grid / 2;
    ...
    float _ans = ans->x;
    _ans = GetLarger(_ans, ans->y);
    _ans = GetLarger(_ans, ans->z);
    _ans = GetLarger(_ans, ans->w);
    *res = _ans;
    delete ans;
...
void decx::Max_fp32_mat(_Matrix<float>* src, float* res, de::DH* handle)
{
    // 2x float4 as a pair of adding operation
    ...
    while (1) {
        grid = decx::utils::ceil<size_t>(thr_num, REDUCTION_VEC4_BLOCK);
        if (dev_A.leading) {
            cu_max_vec4f << <grid, REDUCTION_VEC4_BLOCK, 0, S >> > (dev_A.mem, dev_B.mem, thr_num, src->Mat.ptr[0]);
            decx::utils::set_mutex_memory_state<float4, float4>(&dev_B, &dev_A);
        }
        else {
            cu_max_vec4f << <grid, REDUCTION_VEC4_BLOCK, 0, S >> > (dev_B.mem, dev_A.mem, thr_num, src->Mat.ptr[0]);
            decx::utils::set_mutex_memory_state<float4, float4>(&dev_A, &dev_B);
        }
        thr_num = grid / 2;
...
de::DH de::Global_Max(de::Vector<float>& src, float* res)
{
    ...
    decx::Max_fp32_vec(_src, res, &handle);
    return handle;
}


de::DH de::Global_Max(de::Matrix<float>& src, float* res)
{
    ...
    decx::Max_fp32_mat(_src, res, &handle);
    return handle;
}