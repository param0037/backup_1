...
void decx::dev_transpose_4x4(_GPU_Matrix<_type>* src, _GPU_Matrix<_type>* dst, de::DH* handle)
{
    ...
    cu_transpose_vec4x4<<<grid, block, 0, S>>>(dev_src, dev_dst, src->pitch / 4, dst->pitch / 4, make_uint2(src->width, src->height));
...
void decx::dev_transpose_h8x8(_GPU_Matrix<de::Half>* src, _GPU_Matrix<de::Half>* dst, de::DH* handle)
{
    ...
    cu_transpose_vec8x8 << <grid, block, 0, S >> > (dev_src, dev_dst, src->pitch / 8, dst->pitch / 8, make_uint2(src->width, src->height));
...
void decx::dev_transpose_2x2(_GPU_Matrix<_type>* src, _GPU_Matrix<_type>* dst, de::DH* handle)
{
    ...
    cu_transpose_vec2x2 << <grid, block, 0, S >> > (dev_src, dev_dst, src->pitch / 2, dst->pitch / 2, make_uint2(src->width, src->height));
