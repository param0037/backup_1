...
void de::vis::Canny_Sobel(de::vis::Img& src, de::vis::Img& dst, const float _L_thr, const float _H_thr, const uint step)
{
	...
	// ~.x : width; ~.y : height
	const dim3 srcDim(src.width, src.height);
	// ~.x : width; ~.y : height
	...
	GI* GI_tmp;
	size_t src_pitch;
	size_t srcWidth = src.width * sizeof(uchar);
	checkCudaErrors(cudaMallocPitch(reinterpret_cast<void**>(&dev_src), &src_pitch, srcWidth, src.height));
	...
	checkCudaErrors(cudaMemcpy2DAsync(dev_src, src_pitch, src.Mat, srcWidth, srcWidth, src.height, cudaMemcpyHostToDevice, S_C));
	...
	checkCudaErrors(cudaHostGetDevicePointer(&dev_dst, dst.Mat, 0));
